"""
配置文件示例

复制此文件为 config.py 并根据你的需求修改参数。
注意: config.py 已在 .gitignore 中，不会被提交到版本控制。

Author: AI Assistant
Date: 2025-10-05
"""

# ==============================================================================
# 数据输入配置
# ==============================================================================

# CSV文件所在的根目录（会递归搜索所有子目录）
INPUT_DIR = "/path/to/your/csv/files"

# CSV文件表头格式（默认）
CSV_COLUMNS = ["x [nm]", "y [nm]", "z [nm]"]

# ==============================================================================
# 样本生成配置
# ==============================================================================

# 需要生成的样本数量
NUM_SAMPLES = 100

# 提取区域的大小（单位：nm）
# 格式: (x_size, y_size, z_size)
REGION_SIZE = (1000.0, 1000.0, 500.0)

# 期望的点数（每个样本的目标点数）
# 如果实际点数不足，会随机复制点；如果过多，会随机删除点
TARGET_POINTS = 1024

# 每个样本的最小点数要求
# 如果提取的区域点数少于此值，会自动重试
MIN_POINTS = 100

# 每个CSV文件的最大尝试次数
# 如果连续尝试都失败，则放弃该CSV文件
MAX_ATTEMPTS = 10

# 坐标精度（保留的小数位数）
DECIMALS = 2

# 随机种子（用于结果可重复性）
# 设置为 None 则每次运行结果不同
RANDOM_SEED = 42

# ==============================================================================
# 输出配置
# ==============================================================================

# 输出H5文件路径
OUTPUT_H5 = "outputs/processed_data/pointcloud_samples.h5"

# H5文件中数据集的名称
DATASET_NAME = "pointclouds"

# H5文件压缩方法
# 可选: "gzip", "lzf", None
COMPRESSION = "gzip"

# ==============================================================================
# 测试输出配置（用于测试脚本）
# ==============================================================================

# 测试输出目录
TEST_OUTPUT_DIR = "outputs/test_processed_data"

# 测试样本数量（建议使用较小的值）
TEST_NUM_SAMPLES = 10

# 可视化参数
VISUALIZATION = {
    "figsize": (8, 8),
    "dpi": 150,
    "cmap": "viridis",
    "point_size": 1.0,
    "show_colorbar": True,
    "show_stats": True
}

# ==============================================================================
# 数据增强配置（高级）
# ==============================================================================

# 是否启用随机旋转（绕Z轴）
ENABLE_ROTATION = True

# 旋转角度范围（弧度）
# None 表示 [0, 2π] 的均匀分布
ROTATION_ANGLE_RANGE = None

# 是否对区域提取位置进行约束
# 例如: 只从点云中心区域提取
USE_CENTER_BIAS = False

# 中心偏移系数（0.0-1.0）
# 0.0 表示完全随机，1.0 表示只从中心提取
CENTER_BIAS_FACTOR = 0.5

# ==============================================================================
# 日志配置
# ==============================================================================

# 日志级别
# 可选: "DEBUG", "INFO", "WARNING", "ERROR"
LOG_LEVEL = "INFO"

# 是否将日志保存到文件
SAVE_LOG_TO_FILE = True

# 日志文件路径
LOG_FILE = "outputs/logs/data_processing.log"
