...
class CSVPointCloudProcessor:
    ...
    def load_and_process(self):
        ...
        self.volume_dims = np.ceil(translated_max).astype(int)
...
def main_inference():
    ...
    # 输入输出参数
    parser.add_argument('--model-path', type=str, default='/repos/coarse2fine-pcgen/refine-exp/checkpoints/iteration_260000.pt',
                        help='训练好的模型路径')
    parser.add_argument('--csv-path', type=str, default='/repos/datasets/particles_XYZ.csv',
                        help='输入CSV点云文件路径')
    parser.add_argument('--output-dir', type=str, default='/repos/datasets/particles_XYZ',
                        help='推理结果输出目录')
